...
void TestNetworkDao::testGetDefaultNeuronParameters(){
	try{
		...
		QCOMPARE(resultsMap["sigma"], 0.0); 
			// .\database\AddNeuronTypes.sql
			// sigma DOUBLE DEFAULT 0.0 COMMENT 'Parameter for a random gaussian per-neuron process, 
			// which generates random input current drawn from an N(0,\a sigma) distribution.
			// If set to zero no random input current will be generated.',

		QVERIFY(resultsMap.contains("seed"));
		QCOMPARE(resultsMap["seed"], 45.0);
			// .\database\AddNeuronTypes.sql
			//	seed DOUBLE DEFAULT 45 COMMENT 'Seed for the random number generator.		
	}
	catch(SpikeStreamException ex){
		QFAIL(ex.getMessage().toLatin1());
	}
...
void TestNetworkDao::testGetDefaultSynapseParameters(){
	try{
		...
		//Check the default values of the parameters are correct
		QCOMPARE(resultsMap.size(), (int)3); //Fix 26.10.2013
		...
		//Fix 26.10.2013
		QVERIFY(resultsMap.contains("weight_factor"));
		QCOMPARE(resultsMap["weight_factor"], 10.0);
	}
	catch(SpikeStreamException ex){
		QFAIL(ex.getMessage().toLatin1());
	}
...
void TestNetworkDao::testGetNeuronParameters(){
	...
	try{
		...
		QCOMPARE(neurParamMap["sigma"], 0.0); //Fixed TestCase
		QVERIFY(neurParamMap.contains("seed"));
		QCOMPARE(neurParamMap["seed"], 45.0); //Fixed TestCase
	}
	catch(SpikeStreamException ex){
		QFAIL(ex.getMessage().toLatin1());
	}
...
void TestNetworkDao::testGetNeuronType2(){
	try{
		NetworkDao networkDao(networkDBInfo);

		//Test type 4 neurons
		NeuronType type = networkDao.getNeuronType(4);
		QCOMPARE(type.getDescription(), QString("Dopaminergic Neuron"));
		QCOMPARE(type.getParameterTableName(), QString("DopaminergicNeuronParameters"));
		QCOMPARE(type.getParameterInfoList().size(), (int)8+2);
		QCOMPARE(type.getParameterInfoList().at(8).getName(), QString("da_baseline"));
		QCOMPARE(type.getParameterInfoList().at(8).getDescription(), QString("baseline of the DA level"));
		QCOMPARE(type.getParameterInfoList().at(9).getName(), QString("da_tau"));
		QCOMPARE(type.getParameterInfoList().at(9).getDescription(), QString("decay rate of the DA concentration"));

		type = networkDao.getNeuronType(5);
		QCOMPARE(type.getDescription(), QString("Custom Excitatory Neuron"));
		QCOMPARE(type.getParameterTableName(), QString("CustomExcitatoryNeuronParameters"));
		QCOMPARE(type.getParameterInfoList().size(), (int)0);

		type = networkDao.getNeuronType(6);
		QCOMPARE(type.getDescription(), QString("Custom Inhibitory Neuron"));
		QCOMPARE(type.getParameterTableName(), QString("CustomInhibitoryNeuronParameters"));
		QCOMPARE(type.getParameterInfoList().size(), (int)0);

		type = networkDao.getNeuronType(7);
		QCOMPARE(type.getDescription(), QString("Poisson Excitatory Neuron"));
		QCOMPARE(type.getParameterTableName(), QString("PoissonExcitatoryNeuronParameters"));
		QCOMPARE(type.getParameterInfoList().size(), (int)4);
		QCOMPARE(type.getParameterInfoList().at(0).getName(), QString("Generator"));
		QCOMPARE(type.getParameterInfoList().at(0).getDescription(), QString("0 no generator = see Custom, 1 Poisson, 2 Periodical, mean = actual , see 6.1"));
		QCOMPARE(type.getParameterInfoList().at(1).getName(), QString("SpikeAtZero"));
		QCOMPARE(type.getParameterInfoList().at(2).getName(), QString("Rate"));
		QCOMPARE(type.getParameterInfoList().at(3).getName(), QString("Rates"));


		type = networkDao.getNeuronType(8);
		QCOMPARE(type.getDescription(), QString("Poisson Inhibitory Neuron"));
		QCOMPARE(type.getParameterTableName(), QString("PoissonInhibitoryNeuronParameters"));
		QCOMPARE(type.getParameterInfoList().size(), (int)4);
		QCOMPARE(type.getParameterInfoList().at(0).getName(), QString("Generator"));
		QCOMPARE(type.getParameterInfoList().at(3).getName(), QString("Rates"));
...
void TestNetworkDao::testGetNeuronTypes(){
	...
	try{
		...
		QVERIFY(neurTypesList.size() >= 3); // new NeuronTypes
		...
		QCOMPARE(neurTypesList.at(2).getClassLibaryName(), QString(""));
	}
	catch(SpikeStreamException ex){
		QFAIL(ex.getMessage().toLatin1());
	}
...
/*! Note this test will need to be updated if the default types change. */
void TestNetworkDao::testGetNeuronTypes2(){
	//Invoke method being tested
	NetworkDao networkDao(networkDBInfo);
	try{
		QList<NeuronType> neurTypesList = networkDao.getNeuronTypes();
		QVERIFY(neurTypesList.size() >= 3+5); // new NeuronTypes

		//Check ids
		for(unsigned int i=3;i<8;i++) 
			QCOMPARE(neurTypesList.at(i).getID(), i+1);

		//Check descriptions
		QCOMPARE(neurTypesList.at(3).getDescription(), QString("Dopaminergic Neuron"));
		QCOMPARE(neurTypesList.at(4).getDescription(), QString("Custom Excitatory Neuron"));
		QCOMPARE(neurTypesList.at(5).getDescription(), QString("Custom Inhibitory Neuron"));
		QCOMPARE(neurTypesList.at(6).getDescription(), QString("Poisson Excitatory Neuron"));
		QCOMPARE(neurTypesList.at(7).getDescription(), QString("Poisson Inhibitory Neuron"));

		//Check parameter table names
		QCOMPARE(neurTypesList.at(3).getParameterTableName(), QString("DopaminergicNeuronParameters"));
		QCOMPARE(neurTypesList.at(4).getParameterTableName(), QString("CustomExcitatoryNeuronParameters"));
		QCOMPARE(neurTypesList.at(5).getParameterTableName(), QString("CustomInhibitoryNeuronParameters"));
		QCOMPARE(neurTypesList.at(6).getParameterTableName(), QString("PoissonExcitatoryNeuronParameters"));
		QCOMPARE(neurTypesList.at(7).getParameterTableName(), QString("PoissonInhibitoryNeuronParameters"));

		//Check class names
		for(int i=3;i<8;i++) 
			QCOMPARE(neurTypesList.at(i).getClassLibaryName(), QString(""));
...
void TestNetworkDao::testGetSynapseParameters(){
	...
	try{
		...
		//Check parameters
		QCOMPARE(conParamMap.size(), (int)3); //Fix 26.10.2013
		...
		//Fix 26.10.2013
		QVERIFY(conParamMap.contains("weight_factor"));
		QCOMPARE(conParamMap["weight_factor"], 10.0);
	}
	catch(SpikeStreamException ex){
		QFAIL(ex.getMessage().toLatin1());
	}
...
void TestNetworkDao::testGetSynapseType(){
	try{
		...
		QCOMPARE(type.getParameterInfoList().size(), (int)3); //Fix 26.10.2013
		...
		//Fix 26.10.2013
		QCOMPARE(type.getParameterInfoList().at(2).getName(), QString("weight_factor")); 
		QCOMPARE(type.getParameterInfoList().at(2).getDescription(), QString("Factor by which weights are multiplied for the simulation. When set to 10 it typically takes two synchronous spikes to fire an Izhikevich neuron."));
...
void TestNetworkDao::testGetSynapseTypes(){
	...
	try{
		...
		//Check parameters
		QCOMPARE(synTypesList.at(0).getParameterInfoList().size(), (int)3);//Fix 26.10.2013 
...
void TestNetworkDao::testSetNeuronParameters(){
	...
	try{
		...
		QCOMPARE(resultsMap["v"], -65.0);
		QVERIFY(resultsMap.contains("sigma"));
		QCOMPARE(resultsMap["sigma"], 0.0);//Excpected value corrected for actual db schema
		QVERIFY(resultsMap.contains("seed"));
		QCOMPARE(resultsMap["seed"], 45.0);//Excpected value corrected for actual db schema
...
void TestNetworkDao::testSetSynapseParameters(){
	...
	try{
		...
		paramMap["Learning"] = 1;
		paramMap["Disable"] = 1;
		paramMap["weight_factor"] = 8.5;//Fix 26.10.2013 

		//Check values of parameters
		NetworkDao netDao(networkDBInfo);
		QHash<QString, double> resultsMap = netDao.getSynapseParameters(info);
		QCOMPARE(resultsMap.size(), (int)3);//Fix 26.10.2013 Excpected value corrected for actual db schema
		QVERIFY(resultsMap.contains("Learning"));
		QVERIFY(resultsMap.contains("Disable"));
		QCOMPARE(resultsMap["Learning"], 0.0);
		QCOMPARE(resultsMap["Disable"], 0.0);
		...
		QCOMPARE(resultsMap.size(), (int)3);//Fix 26.10.2013 Excpected
		QVERIFY(resultsMap.contains("Learning"));
		QVERIFY(resultsMap.contains("Disable"));
		...
		//Fix 26.10.2013
		QVERIFY(resultsMap.contains("weight_factor"));
		QCOMPARE(resultsMap["weight_factor"], 8.5);
	}
	catch(SpikeStreamException ex){
		QFAIL(ex.getMessage().toLatin1());
	}
