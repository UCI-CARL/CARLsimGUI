...
void TestNetwork::testGetBoundingBox(){
	...
	QCOMPARE(box.x2, 9.0f); //Fixed expected value, \sa TestDao::addTestNetwork1()
    QCOMPARE(box.y1, -7.0f);
	//QCOMPARE(box.y2, -0.9f);
	QCOMPARE(box.y2, -1.0f);//Fixed expected value, \sa TestDao::addTestNetwork1()
    QCOMPARE(box.z1, -6.0f);
	//QCOMPARE(box.z2, 10.1f);
	QCOMPARE(box.z2, 10.0f);//Fixed expected value, \sa TestDao::addTestNetwork1()
}
...
//	network.loadWait();
//	if(network.isError())
//		QFAIL(network.getErrorMessage().toLatin1());
