...
#ifndef LN_FIX_PROTOTYPE_IDS  // !! CAUTION, is for connecitons
//Initialize static variables
unsigned NeuronGroup::neuronIDCounter = LAST_NEURON_ID + 1;
//unsigned NeuronGroup::neuronIDCounter = 0;
#else
unsigned NeuronGroup::neuronIDCounter = -1;  // first Group has 0 as ID  
#endif
...
//#else
	// assignment seems not work for Neurons, the value is screwed
	// it should work:
	// https://doc.qt.io/qt-6/qmap.html
	//   To insert a (key, value) pair into the map, you can use operator[]():
	// however 
	// it does not: STD exception accurred during event handling: bad arra new length
	//(*neuronMap)[tmpID] = tmpNeuron;  
	//same with insert
	//neuronMap->insert(tmpID, tmpNeuron);
//#endif
...
/*! Adds a layer to the group with the specified width and z-height.
\note Maps height to the z-axis which is consistent to Box and Position 
Temporary neuron ids are used and the neurons are appended to the neurons already in the group. */
void NeuronGroup::addLayer2(int width, int zheight, int xPos, int yPos, int zPos) {
	for (int x = xPos; x < (xPos + width); ++x) {
		for (int z = zPos; z < (zPos + zheight); ++z) {
			addNeuron(x, yPos, z);
		}
	}
	neuronGroupChanged();
...
/*! Returns a 64 bit key that encodes the position of the neuron in 3D.
	...
	\Remark: For ordering the keys, x is therefore the most significant component in the key, 
	followed by y. Therefore the order sequence is z -> y -> x which suites the 
	interface layer well to scalaras are mapped to z columns and depths is 1 */
...
Point3D NeuronGroup::getPointFromPositionKey(uint64_t positionKey){
	uint64_t keyExtractor = 2097151; //0b111111111111111111111 (21bits)
...
uint64_t NeuronGroup::getPositionKey(int xPos, int yPos, int zPos) {
	if (xPos < 0 || yPos < 0 || zPos < 0)
		throw SpikeStreamException("This method currently only works with positive positions.");
	// this would keep the order but shift the range from [0,2^21-1] to [-2^20,2^20-1]
...
void NeuronGroup::initializeFiring() { 
	this->firing.resize(this->size(), false); 
}


void NeuronGroup::setFiring(const std::vector<unsigned int>& aer) {
