...
bool Network::containsNeuron(unsigned int neurID){
    ...
    for(QCONTAINER_NEURONGROUP::iterator iter = neurGrpMap.begin(); iter != neurGrpMap.end(); ++iter){
...
int Network::getConnectionCount(unsigned conGrpID){
	...
	return (int) connGrpMap[conGrpID]->size(); // 0.3
}
...
QList<ConnectionGroup*> Network::getConnectionGroups(){
	...
	for(QCONTAINER_CONNECTIONGROUP::iterator iter = connGrpMap.begin(); iter != connGrpMap.end(); ++iter)
...
/*! Returns a map of the neuron connection groups in the network.*/
QHash<QString, ConnectionGroup*> Network::getConnectionGroupsMap() {
	QHash<QString, ConnectionGroup*> tmpMap;
	for (QCONTAINER_CONNECTIONGROUP::iterator iter = connGrpMap.begin(); iter != connGrpMap.end(); ++iter) {
		tmpMap[iter.value()->getInfo().getDescription()] = (iter.value());
	}
	return tmpMap;
}
...
/*! Returns a map of the neuron groups in the network.*/
QHash<QString, NeuronGroup*> Network::getNeuronGroupsMap() {
	QHash<QString, NeuronGroup*> tmpMap;
	for (QCONTAINER_NEURONGROUP::iterator iter = neurGrpMap.begin(); iter != neurGrpMap.end(); ++iter) {
		tmpMap[iter.value()->getInfo().getName()] = (iter.value());
	}		
	return tmpMap;
}
...
#else
	//Remove connection and neuron groups from network - they will be added later with the correct IDs.
	for (QCONTAINER_CONNECTIONGROUP::iterator iter = newConnectionGroupMap.begin(); iter != newConnectionGroupMap.end(); ++iter)
		connGrpMap.remove(iter.value()->getID());
	for (QCONTAINER_NEURONGROUP::iterator iter = newNeuronGroupMap.begin(); iter != newNeuronGroupMap.end(); ++iter)
		neurGrpMap.remove(iter.value()->getID());
#endif
...
#else
		for (QMap<unsigned int, NeuronGroup*>::iterator iter = neurGrpMap.begin(); iter != neurGrpMap.end(); ++iter) {
#endif
...
#else
/*! Returns an temporary ID connection group ID starting at 0. 
	The mentioned of conflict with the database IDs is highly likely obsolete. */
unsigned Network::getTemporaryConGrpID() {
	unsigned tmpID = 0;
	while (connGrpMap.contains(tmpID)) {
		++tmpID;
		if (tmpID == 0xffffffff)
			throw SpikeStreamException("Cannot find temporary connection ID");
	}
	return tmpID;
}


/*! Returns an temporary ID neuron group ID starting at 0. 
	The mentioned of conflict with the database IDs is highly likely obsolete. */
unsigned Network::getTemporaryNeurGrpID() {
	unsigned tmpID = 0;
	while (neurGrpMap.contains(tmpID)) {
