...
#else
		// FIXED
		auto endNeurGrp = neuronGroup->positionEnd();
		for (auto neurIter = neuronGroup->positionBegin(); neurIter != endNeurGrp && !stopThread; ++neurIter) {
#endif
...
#else
		//Store old neuron IDS
		NeuronMap::iterator endNeurGrp = tmpNeurGrp->end();
		for (NeuronMap::iterator neurIter = tmpNeurGrp->begin(); neurIter != endNeurGrp; ++neurIter) {
			oldIDNeurMap[neurIter.key()] = neurIter.value();
		}
#endif
	}
...
void NetworkDaoThread::saveTempWeights(){
	...
	for(QList<ConnectionGroup*>::iterator conGrpIter = volatileConnectionGroupList.begin(); conGrpIter != volatileConnectionGroupList.end() && !stopThread; ++conGrpIter)
		totalNumberOfSteps += (int) (*conGrpIter)->size(); //0.3
