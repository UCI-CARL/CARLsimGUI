...
bool DBConfigMainWindow::addArchiveDatabases(const DBInfo& archiveDBInfo){
	...
	foreach (QString dbName, dbList){
		...
		//if(dbName.toLower() == archiveDBInfo.getDatabase()){
			bool ok = showConfirmMessage("A SpikeStreamArchive database already exists at this location. Would you like to reset it?\nAll data will be lost. This step cannot be undone.");
			//bool ok = showConfirmMessage(
			//		QString().sprintf("The database %s already exists at this location. Would you like to reset it?\nAll data will be lost. This step cannot be undone.", 
			//			archiveDBInfo.getDatabase().toStdString().c_str()) );
			if(!ok)
				return false;
			break;
		}
...
void DBConfigMainWindow::writeDBInfoToConfigFile(bool writeNetDB, const DBInfo& networkDBInfo, bool writeArchDB,  const DBInfo& archiveDBInfo, bool writeAnaDB, const DBInfo& analysisDBInfo){
	...
	if(writeNetDB){
		...
		//configParams["spikeStreamNetworkDatabase"] = networkDBInfo.getDatabase();  // Named are fixed per design
	}
	if(writeArchDB){
		...
		//configParams["spikeStreamArchiveDatabase"] = archiveDBInfo.getDatabase();
	}
	if(writeAnaDB){
		...
		//configParams["spikeStreamAnalysisDatabase"] = analysisDBInfo.getDatabase();
	}
