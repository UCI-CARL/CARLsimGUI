...
DBDetailsWidget::DBDetailsWidget(QWidget* parent) : QWidget(parent){
	...
	try{
		...
		//ConfigLoader configLoader(Globals::getConfigFile());
		...
		//networkDatabase = new QLineEdit(configLoader.getParameter("spikeStreamNetworkDatabase", "SpikeStreamNetwork"));
		...
		//netGridLayout->addWidget(new QLabel("Database "), 4, 0);
		//netGridLayout->addWidget(networkDatabase, 4, 1);
		...
		//archiveDatabase = new QLineEdit(configLoader.getParameter("spikeStreamArchiveDatabase", "SpikeStreamArchive"));
		...
		//archGridLayout->addWidget(new QLabel("Database "), 4, 0);
		//archGridLayout->addWidget(archiveDatabase, 4, 1);
		...
		//analysisDatabase = new QLineEdit(configLoader.getParameter("spikeStreamAnalysisDatabase", "SpikeStreamAnalysis"));
		...
		//anaGridLayout->addWidget(new QLabel("Database "), 4, 0);
		//anaGridLayout->addWidget(analysisDatabase, 4, 1);
...
void DBDetailsWidget::getDBInformation(){
	...
	try{
		if(netChkBox->isChecked()){
			...
			//netDBInfo.setDatabase(getDBInfoString(networkDatabase));
		}
		if(archChkBox->isChecked()){
			...
			//archDBInfo.setDatabase(getDBInfoString(archiveDatabase));
		}
		if(anaChkBox->isChecked()){
			...
			//anaDBInfo.setDatabase(getDBInfoString(analysisDatabase));
		}
	}
	catch(SpikeStreamException& ex){
		qCritical()<<"Information about the host, username or password is missing.";
		qCritical()<<ex.getMessage(); // 0.3: warning C4101: 'ex' : unreferenced local variable
		return;
	}
