...
CuboidWidget::CuboidWidget(QWidget* parent) : QWidget(parent) {
	...
	//Add spacing and density inputs
	conductancesEdit = new QLineEdit("0");
	conductancesEdit->setMaximumSize(100, 30);
	conductancesEdit->setValidator(positiveIntValidator);
	QHBoxLayout* carlsimLayout = new QHBoxLayout();
	carlsimLayout->addWidget(new QLabel("Conductances: "));
	carlsimLayout->addWidget(conductancesEdit);
	carlsimLayout->addStretch(5);
	mainVBox->addLayout(carlsimLayout);
	mainVBox->addSpacing(5);
#endif
...
#ifdef CARLSIMGUI_USES_CARLSIM
	int conductances = Util::getDouble(conductancesEdit->text());

	paramMap["Conductances"] = conductances;  // CARLsim Conductances
#endif
