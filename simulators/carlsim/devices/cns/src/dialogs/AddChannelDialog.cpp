...
AddChannelDialog::~AddChannelDialog(){
	...
	if(inboundFactory != NULL)
		delete inboundFactory;
	if(outboundFactory != NULL)
		delete outboundFactory;
...
void AddChannelDialog::configureChannel(){
	...
	try{
		...
		if(inboundChannel){
			...
			string channelName = inChanDesc[channelCombo->currentIndex()].getName();
			bool propsConfigured = configureProperties(inboundFactory->getDefaultProperties(inChanDesc[channelCombo->currentIndex()]), channelProperties, true, channelName);
			if(!propsConfigured)
				return;
			...
			//Edit properties of channel - abort channel configuration if operation is cancelled
			string channelName = outChanDesc[channelCombo->currentIndex()].getName();
			bool propsConfigured = configureProperties(outboundFactory->getDefaultProperties(outChanDesc[channelCombo->currentIndex()]), channelProperties, true, channelName);
			if(!propsConfigured)
				return;
		...
		qCritical()<<"CARLsimIOException thrown configuring channel: "<<ex.what();
	}
	catch(...){
...
void AddChannelDialog::configureReaderWriter(){
	try{
		...
		qCritical()<<"CARLsimIOException thrown configuring reader or writer: "<<ex.what();
	}
	catch(...){
...
void AddChannelDialog::okButtonClicked(){
	try{
		if(inboundChannel){
			...
			newInboundChannel = inboundFactory->create(inChanDesc[channelCombo->currentIndex()], newReader, channelProperties);
		}
		else {
			...
			newOutboundChannel = outboundFactory->create(outChanDesc[channelCombo->currentIndex()], newWriter, channelProperties);
		}
		this->accept();  
	} 
	catch(SpikeStreamException& ex){
		...
		qCritical()<<"CARLsimIOException thrown creating channel and reader/writer: "<<ex.what();
	}
	catch (exception& e) {
...
void AddChannelDialog::selectChannelType(){
	...
	try{
		...
		if(channelTypeCombo->currentText() == "Inbound"){
			inboundChannel = true;
			//Get the inbound channels
			inChanDesc = inboundFactory->getAllChannels();
			for(size_t i=0; i<inChanDesc.size(); ++i){
				channelCombo->addItem(QString(inChanDesc[i].getName().data()));
			}
		}
		else if(channelTypeCombo->currentText() == "Outbound"){
			inboundChannel = false; 
			//Get the outbound channels
			outChanDesc = outboundFactory->getAllChannels();
		...
		qCritical()<<"CARLsimIOException thrown selecting channel type: "<<ex.what();
	}
	catch(...){
...
void AddChannelDialog::buildGUI(){
	...
	channelTypeCombo->addItem("Inbound");
	channelTypeCombo->addItem("Outbound");
	...
	//Cns classes
	outboundFactory = new OutboundChannelFactory();
	inboundFactory = new InboundChannelFactory();
...
bool AddChannelDialog::configureProperties(map<string, Property> srcPropertyMap, map<string, Property>& destPropertyMap, bool selectNeuronGroup, string channelName){
	try{
		if(selectNeuronGroup){						
			EditPropertiesDialog* tmpDlg = new EditPropertiesDialog(channelName, srcPropertyMap, false, Globals::getNetwork()->getNeuronGroups());
		...
		qCritical()<<"CARLsimIOException thrown configuring properties: "<<ex.what();
	}
	catch (exception& e) {
