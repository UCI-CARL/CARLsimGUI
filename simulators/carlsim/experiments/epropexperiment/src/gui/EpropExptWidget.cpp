...
EpropExptWidget::EpropExptWidget(QWidget* parent) : AbstractExperimentWidget(parent){
	...
	QPushButton* costsButton = new QPushButton();
	costsButton->setText("Traversal costs...");
	connect(costsButton, SIGNAL(clicked(bool)), this, SLOT(editCosts()));
	toolBar->addWidget(costsButton);
...
// vte is placed in a subdirectory as it does not depend on the central spikestream config
	...
	auto configDir = configFile.absoluteDir().absolutePath() + +"/eprop";
	...
	for (QDirIterator confFileIter(configDir, { "*.config" }, QDir::Files); confFileIter.hasNext(); )
	{
		...
		epropConfigLoaders.append(configLoader);
	...
	vteExptManager = new EpropExptManager();
	...
	connect(vteExptManager, SIGNAL(simulationReloaded()), this, SLOT(updateMonitor()));
...
/*! Destructor */
EpropExptWidget::~EpropExptWidget(){
}
...
void EpropExptWidget::setWrapper(void *wrapper){
//void EpropExptWidget::setWrapper(void *wrapper, CarlsimWrapperWidget* carlsimWrapperWidget){
...
/*------                PUBLIC SLOTS                ------*/
/*----------------------------------------------------------*/
void EpropExptWidget::assignOatMonitor() {
...
//		1 > EpropExptWidget.obj : error LNK2019 : unresolved external symbol "public: void __cdecl spikestream::carlsim_monitors::CarlsimOatWidget::findMonitor(class spikestream::carlsim_monitors::OatMonitor * &,class QString,class QString)" (? findMonitor@CarlsimOatWidget@carlsim_monitors@spikestream@@QEAAXAEAPEAVOatMonitor@23@VQString@@1@Z) referenced in function "private: virtual void __cdecl spikestream::EpropExptWidget::startExperiment(void)" (? startExperiment@EpropExptWidget@spikestream@@EEAAXXZ)
//		1 > C:\cockroach - ut3\build\src\SpikeStream\simulators\carlsim\experiments\epropexperiment\Debug\epropexperiment.dll : fatal error LNK1120 : 1 unresolved externals
	...
	oatWidget->findMonitor(oatMonitor, "OatSpikeMonitor", "CA1 Place Cells");
...
void EpropExptWidget::updateMonitor() {
	vteExptManager->setMonitor(spikeMonitor);
}


void EpropExptWidget::updateTemplate(int i) {

	configLoader = epropConfigLoaders[i];
...
void EpropExptWidget::editCosts() {
...
void EpropExptWidget::managerFinished(){
	//Check for errors
...
void EpropExptWidget::startExperiment(){
	...
	try{
		vteExptManager->startExperiment(carlsimWrapper, spikeMonitor,
			parameterMap, 
			configLoader->getParameter("name"), // presets (configuration name)
			costs, 
			segments, landmarks, // Boone
			starts, ends, selection, // Morris
			placeCells);
	}
	catch(SpikeStreamException& ex){
		qCritical()<<ex.getMessage();
...
//Inherited from AbstractExperimentWidget
void EpropExptWidget::stopExperiment(){
	vteExptManager->stop();
}


/*! Adds the specified message to the status pane */
void EpropExptWidget::updateStatus(QString statusMsg){
...
/*! Builds the current and default parameter map along with the parameter
	info list. */
void EpropExptWidget::buildParameters(){
	//Reset maps and list
	...
	exptNameList.append("Experiment 0 - Start from fix location");   
	exptNameList.append("Experiment 1 - Train route segments"); 
	exptNameList.append("Experiment 2 - Test landmarks");  
	...
	defaultParameterMap[parameterName] = 20 * 32 + 16 + 7; // 663  // 0;
	parameterMap[parameterName] = defaultParameterMap[parameterName];

	parameterName = "target_neuron";   // Hint: this could be a pattern
	parameterInfoList.append(ParameterInfo(parameterName, "Neuron ID that indicates the target location", ParameterInfo::INTEGER));
	defaultParameterMap[parameterName] = 15 * 32 + 8; // 320 + 160 + 8 = 388 // 15;
	parameterMap[parameterName] = defaultParameterMap[parameterName];
	...
	defaultParameterMap[parameterName] = 250; //  2*32*4  // 50;
	parameterMap[parameterName] = defaultParameterMap[parameterName];
	...
	defaultParameterMap[parameterName] = 1000 * 5.0; // -1;   5s
	parameterMap[parameterName] = defaultParameterMap[parameterName];
	...
	defaultParameterMap[parameterName] = 6;
	parameterMap[parameterName] = defaultParameterMap[parameterName];
...
void EpropExptWidget::checkNetwork(){
