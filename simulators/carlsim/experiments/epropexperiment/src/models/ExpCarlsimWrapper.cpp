...
ExpCarlsimWrapper::ExpCarlsimWrapper(CarlsimWrapper* wrapper, carlsim_monitors::OatSpikeMonitor* monitor,
	Maze* maze, int grpId) :
	wrapper(wrapper), maze(maze), monitor(monitor), grpId(grpId),
	iteration(0), loss(-1) {
...
ExpCarlsimWrapper::~ExpCarlsimWrapper() {
	// reset pointers, but do not fee them 
	wrapper = nullptr; 
	maze = nullptr; 
}

void ExpCarlsimWrapper::setMonitor(carlsim_monitors::OatSpikeMonitor* monitor) {
...
//// when ready (depends on the state, inform .. )
//void ExpCarlsimWrapper::assignOatMonitor(abstractList)
////Delegate search of OAT SpikeMonitor out of the widgets to the wrapper
////monitor = wrapper->getOatMonitor("Spike-Monitor", "CA1 Place Cells");
////OatSpikeMonitor* oatMonitor = nullptr; // FIXME
////NeuronGroup* group;  // must be set explictly 
...
// FIXME 4x4
void ExpCarlsimWrapper::printSpikes(int offset) {
...
bool ExpCarlsimWrapper::getSpike(int r, int c, int &t) {
...
bool ExpCarlsimWrapper::checkTargetFired (int targetNId, unsigned int currentTime) {
	SpikeMonitor* spkMon = monitor->monitor;
	spikes_t& spikes  = spkMon->getSpikeVector2D();
...
bool ExpCarlsimWrapper::findWaveFront(int startNId, int endNId) {
...
void ExpCarlsimWrapper::writePathString(std::string & pathString) {
...
void ExpCarlsimWrapper::writePath(std::string filename) {
...
void ExpCarlsimWrapper::writePath(std::unique_ptr<ResultsCursor> &fileResults) {
...
void ExpCarlsimWrapper::writeEligibility(std::string filename) {
	const int buffer_len = 100;
	char buffer[buffer_len];
	std::FILE* file = std::fopen(filename.c_str(), "w");
...
void ExpCarlsimWrapper::writeEligibility(std::unique_ptr<ResultsCursor> &fileResults) {
	std::string path;  // instanciate local variable
	fileResults->getFilePath(path);
...
void ExpCarlsimWrapper::writeCosts(std::string filename) 
{
	const int buffer_len = 100;
	char buffer[buffer_len];
	std::FILE* file = std::fopen(filename.c_str(), "w");
...
void ExpCarlsimWrapper::writeCosts(std::unique_ptr<ResultsCursor> &fileResults) {
	std::string path;  // instanciate local variable
	fileResults->getFilePath(path);
	writeCosts(path);
}
...
void ExpCarlsimWrapper::getDelays() {
...
//void ExpCarlsimWrapper::setBarrier(int barrier, int border) {
//
...
void ExpCarlsimWrapper::updateDelays() {
