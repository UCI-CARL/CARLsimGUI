...
#ifdef NC_EXTENSIONS_CARLSIM
const unsigned STDPFunctions::DA_STDP = 2;
#endif
...
#ifdef NC_EXTENSIONS_CARLSIM
	else if(functionID == DA_STDP)
		return "DA STDP function.";
#endif
...
#ifdef NC_EXTENSIONS_CARLSIM
/*! Returns the reward associated with the application of STDP learning. */
float STDPFunctions::getDaReward(unsigned functionID){
	checkInitialization();
	checkFunctionID(functionID);
	return functionMap[functionID]->getDaReward();   // this completely screw up the generic concept
}

float STDPFunctions::getSynDecay(unsigned functionID){
	checkInitialization();
	checkFunctionID(functionID);
	return functionMap[functionID]->getSynDecay();   // this completely screw up the generic concept
}

float STDPFunctions::getDaDecay(unsigned functionID){
	checkInitialization();
	checkFunctionID(functionID);
	return functionMap[functionID]->getDaDecay();   // this completely screw up the generic concept
}
...
#ifdef NC_EXTENSIONS_CARLSIM
	functionMap[DA_STDP] = new DaSTDPFunction();
#endif
