...
NetworkViewer::NetworkViewer(QWidget* parent) : QGLWidget(parent) {
	...
	connect(Globals::getEventRouter(), SIGNAL(neuronGroupDisplayChangedSignal()), this, SLOT(refreshNeuronGroups()), Qt::QueuedConnection);
	connect(Globals::getEventRouter(), SIGNAL(neuronGroupDisplayChangedSignal()), this, SLOT(refreshNeurons()), Qt::QueuedConnection);
	connect(Globals::getEventRouter(), SIGNAL(connectionGroupDisplayChangedSignal()), this, SLOT(refreshConnections()), Qt::QueuedConnection);
	connect(Globals::getEventRouter(), SIGNAL(weightsChangedSignal()), this, SLOT(refreshConnections()), Qt::QueuedConnection);
	connect(Globals::getEventRouter(), SIGNAL(delaysChangedSignal()), this, SLOT(refreshConnections()), Qt::QueuedConnection);
	...
	connect(Globals::getEventRouter(), SIGNAL(refreshSignal()), this, SLOT(refresh()), Qt::QueuedConnection);

	//Initialize variables
	paintGLSkipped = false;
	resizeGLSkipped = false;
	useAxesDisplayList = false;
	useNeuronGroupsDisplayList = false;
...
void NetworkViewer::initializeGL(){
	...
	neuronGroupsDisplayList = glGenLists(1);
...
void NetworkViewer::mouseDoubleClickEvent (QMouseEvent* event){
	...
	const int SELECT_BUFFER_SIZE = 512;  // patch
...
void NetworkViewer::mousePressEvent (QMouseEvent* event) {
	...
	//// Release
	////Determine if modifier button is down
	//bool ctrlBtnDown = false;
	//if (event->modifiers() & Qt::RightButton) {
	//	ctrlBtnDown = true;
	//}
	//bool shftBtnDown = false;
	...
	if (hitCount != 0) { //Neuron selected
		unsigned int selectedNeuronID = getSelectedNeuron(selectBuffer, hitCount, SELECT_BUFFER_SIZE);
		Globals::getNetworkDisplay()->setContextNeuronID(selectedNeuronID);
	}
	else {
		Globals::getNetworkDisplay()->setContextNeuronID(0); // reset context neuron
	}
...
void NetworkViewer::mouseReleaseEvent (QMouseEvent* event) {
	...
	// handle context 
	Globals::getNetworkDisplay()->handleContext(ctrlBtnDown, shftBtnDown);
...
/*! Redraws the neurons */
void NetworkViewer::refreshNeuronGroups() {
	useNeuronGroupsDisplayList = false;
	updateGL();
}
...
void NetworkViewer::checkOpenGLErrors(){
	...
	GLenum err = glGetError();  // https://www.khronos.org/opengl/wiki/OpenGL_Error
	if(err != GL_NO_ERROR){
		//May be more than one error message, so add them all to the error string if there is an error
		QString errMsg = "OpenGL ERROR(S): ";
		int max_errors = 10;
		while(err != GL_NO_ERROR && --max_errors>0){			
			errMsg += QString("0x%1: %2; ").arg(err, 4, 16, QLatin1Char('0')).arg((const char*)gluErrorString(err));
			err = glGetError();
		}
		if(max_errors<=0)
			errMsg += QString("CAUTION: Max GL Errors reached, see NetworkViewer::checkOpenGLErrors");
...
void NetworkViewer::drawAxes(void){
	...
	//Create new display list with axes
		...
		//Set the drawing colour to gold  
...
void NetworkViewer::drawConnections(){
	...
	//Create new display list with connections
		...
		unsigned delayRenderMode = netDisplay->getDelayRenderMode();
		float delay = 0.0f;//Declare here to save declaring it all the time.
		...
		float connectionAlpha = netDisplay->getConnectionTransparency(); 
		...
		QPair<float, float> weightXdelay;
		for(QList<unsigned int>::iterator conGrpIter = conGrpIDs.begin(); conGrpIter != conGrpIDs.end(); ++conGrpIter){
			...
			size_t numCons = conGrp->size(); // 0.3
			...
			for(ConnectionIterator conIter = conGrp->begin(); conIter != endConGrp; ++ conIter){
				...
				delay = (weight > .0f ? 1.f : -1.f) * 2.0f / conIter->getDelay();  // simpliest form d=1ms, max thikness, 2ms half, ..
				if( (weightRenderMode & WEIGHT_RENDER_ENABLED) && (weightRenderMode & RENDER_TEMP_WEIGHTS) )
					weight = conIter->getTempWeight();
				if ((delayRenderMode & DELAY_RENDER_ENABLED) && (delayRenderMode & RENDER_TEMP_DELAYS)) {
					delay = (weight > .0f ? 1.f : -1.f) * 2.0f / conIter->getTempDelay();  // simpliest form d=1ms, max thikness, 2ms half, ..
				}

				weightXdelay.first = weight;
				weightXdelay.second = delay;
					...
					if( !(connectionMode & SHOW_BETWEEN_CONNECTIONS) ){
						...
						if(connectionMode & SHOW_FROM_CONNECTIONS){
							if( conIter->getFromNeuronID() != singleNeuronID){
								...
								connectedNeuronMap[conIter->getToNeuronID()] = weightXdelay;
							}
						}
						//Show only connections to a single neuron
							...
							if( conIter->getToNeuronID() != singleNeuronID){
								...
								connectedNeuronMap[conIter->getFromNeuronID()] = weightXdelay;//Positive connection
							}
						}
						//Show from and to connections to a single neuron
							...
							if( (conIter->getFromNeuronID() != singleNeuronID) && (conIter->getToNeuronID() != singleNeuronID) ){
								...
								if( conIter->getFromNeuronID() == singleNeuronID){
									connectedNeuronMap[conIter->getToNeuronID()] = weightXdelay;//Positive connection
								}
				...
				if(drawConnection){
					...
					if (fullRender && delayRenderMode & DELAY_RENDER_ENABLED) {
						drawDelayedConnection(
							fromNeuronPoint->getXPos(),
							fromNeuronPoint->getYPos(),
							fromNeuronPoint->getZPos(),
							toNeuronPoint->getXPos(),
...
void NetworkViewer::drawNeuronGroups() {
	...
	//Use existing display list
	if (useNeuronGroupsDisplayList) {
		glCallList(neuronGroupsDisplayList);
	}
	//Create new display list with connections
	else {
		glNewList(neuronGroupsDisplayList, GL_COMPILE_AND_EXECUTE);
		...
		for (QList<unsigned int>::iterator neurGrpIter = neuronGrpIDs.begin(); neurGrpIter != neuronGrpIDs.end(); ++neurGrpIter) {
			...
			//Draw the neuron group 
			glPushName(grp->getID());
		...
		//Finished creating neuron  groups display list
		glEndList();
		useNeuronGroupsDisplayList = true;
	}
}
		...
		for(QList<unsigned int>::iterator neurGrpIter = neuronGrpIDs.begin(); neurGrpIter != neuronGrpIDs.end(); ++neurGrpIter){
			...
			for(NeuronMap::iterator neurIter = neuronMap->begin(); neurIter != neurMapEnd; ++neurIter){
				...
				if(connectionMode & CONNECTION_MODE_ENABLED){
					...
					glColor4f(tmpColor->red, tmpColor->green, tmpColor->blue, 1.0f);  
				}
				else{
					glColor4f(defaultNeuronColor.red, defaultNeuronColor.green, defaultNeuronColor.blue, neuronAlpha);
				}
...
void NetworkViewer::drawWeightedConnection(float x1, float y1, float z1, float x2, float y2, float z2, float weight, unsigned weightRenderMode){
	...
	float sphereRadius = Globals::getNetworkDisplay()->getSphereRadius();
	...
	if (weightRenderMode & RENDER_WEIGHT_SYNAPSE) {
		glTranslatef(0.0, 0.0, length - sphereRadius * 3);
		gluCylinder(gluConeObj, 0.0, weightRadiusFactor * weight, sphereRadius, 8, 8);
	} 
	else if (weightRenderMode & RENDER_WEIGHT_EVEN) {
		glTranslatef(0.0, 0.0, sphereRadius);
		gluCylinder(gluConeObj, weightRadiusFactor * weight, weightRadiusFactor * weight, length - 2 * sphereRadius, 8, 8);
	}
	else if (weightRenderMode & RENDER_WEIGHT_MIDDLE) {
		glTranslatef(0.0, 0.0, sphereRadius);
		gluCylinder(gluConeObj, 0.0, weightRadiusFactor * weight, length / 2.0 - sphereRadius, 8, 8);
		glTranslatef(0.0, 0.0, length / 2.0 - sphereRadius);
		gluCylinder(gluConeObj, weightRadiusFactor * weight, 0.0, length / 2.0 - sphereRadius, 8, 8);
	}
	glPopMatrix();
...
/*! Draws a connection whose thickness varies with the weight */
void NetworkViewer::drawDelayedConnection(float x1, float y1, float z1, float x2, float y2, float z2, float weight) {
	...
	glTranslatef(0.0, 0.0, sphereRadius * 1.0); // move up current Z axis to attach to hull
	
	
	// fast
	//// https://www.canva.com/de_de/lernen/goldener-schnitt-einfach-erklaert/
	//gluCylinder(gluConeObj, weightRadiusFactor * weight, weightRadiusFactor * weight, 
	//	length / 1.618 - sphereRadius * 2, 8, 8); // lenth 3/4 

	// slow 
	float sheath  = 1.0 / 1.618 * (length - sphereRadius * 2.0);  // 0.75 or 1.0 / 1.618  or 0.66
	float myelin = 0.0f;
	do {
		gluCylinder(gluConeObj, weightRadiusFactor * weight / 1.618, weightRadiusFactor * weight / 1.618, 0.5 * sphereRadius, 8, 8);
		glTranslatef(0.0, 0.0, 0.5 * sphereRadius);
		gluCylinder(gluConeObj, weightRadiusFactor * weight, weightRadiusFactor * weight, 2.0 * sphereRadius, 8, 8);
		glTranslatef(0.0, 0.0, sphereRadius * 2.0);
		myelin += 2.5 * sphereRadius;
	} while (myelin + 2.5 * sphereRadius < sheath);
