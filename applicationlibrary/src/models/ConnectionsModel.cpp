...
ConnectionsModel::ConnectionsModel() : QAbstractTableModel(){
	...
	connect(Globals::getEventRouter(), SIGNAL(delaysChangedSignal()), this, SLOT(visibleConnectionsChanged()), Qt::QueuedConnection);
}
...
QVariant ConnectionsModel::data(const QModelIndex & index, int role) const{
    ...
    if (role == Qt::DisplayRole){
		...
		if (index.column() == tmpDelayCol)
			return tmpConnection->getTempDelay();
